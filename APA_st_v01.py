...
with col4:
    map = folium.Map([dfgeocommune[dfgeocommune['CODE_COMMUNE']==df2020[df2020['LIBL_COMMUNE']==commune]['CODE_COMMUNE'].tolist()[0]]['LAT'],
                      dfgeocommune[dfgeocommune['CODE_COMMUNE']==df2020[df2020['LIBL_COMMUNE']==commune]['CODE_COMMUNE'].tolist()[0]]['LONG']],
                     zoom_start=10) # carte centrée sur la commune
    folium.vector_layers.Polygon(locations=dfgeocommune[dfgeocommune['CODE_COMMUNE']==df2020[df2020['LIBL_COMMUNE']==commune]['CODE_COMMUNE'].tolist()[0]]['geo'],
                                 fill_color='blue',
                                 popup=commune).add_to(map) # tracé du contour et de la surface de la commune
...
if choix == 'Par sexe':
    ...
    if max(np.array(dom) + np.array(eta))<=10:
        plt.ylim(top=10.5)
    col4.pyplot(fig)
    col4.write("Les données ne sont comptabilisées qu'à partir de 2016")
