"""
Ce module est une application Streamlit
Elle permet de visualiser le nombre de personnes bénéficiant de l'APA dans le Puy de Dôme
"""
#!/usr/bin/env python
# coding: utf-8
...
st.set_page_config(page_title=' APA - Puy de Dôme ',
                   page_icon=None,
                   layout='wide',
...
indice_commune = dfgeocommune.index[dfgeocommune['CODE_COMMUNE'] ==
                                    df2020[df2020['LIBL_COMMUNE'] == commune]['CODE_COMMUNE']
                                    .tolist()[0]].tolist()[0]
...
if commune == 'CLERMONT-FERRAND':
    col1.metric(
                "Hommes",
                str(
                    df2020['TOTAL_HOM'][df2020['LIBL_COMMUNE'] == commune].sum()
                    ),
                str(
                    df2020['TOTAL_HOM'][df2020['LIBL_COMMUNE'] == commune].sum()
    ...
    col1.metric("Femmes",
                str(df2020['TOTAL_FEM']
                    [df2020['LIBL_COMMUNE'] == commune].sum()),
...
code_commune = int(df2020[df2020['LIBL_COMMUNE'] ==
                   commune]['CODE_COMMUNE'][0:1].values)
...
if code_commune == 63255:
    ...
    for a in année[2:]:
        total.append(int(dfAPA63['TOTAL'][(dfAPA63['ANNEE_REF'] == a) & (
            dfAPA63['CODE_COMMUNE'] == code_commune)].values.sum()))
        fem.append(int(dfAPA63['TOTAL_FEM'][(dfAPA63['ANNEE_REF'] == a) & (
            dfAPA63['CODE_COMMUNE'] == code_commune)].values.sum()))
        hom.append(int(dfAPA63['TOTAL_HOM'][(dfAPA63['ANNEE_REF'] == a) & (
            dfAPA63['CODE_COMMUNE'] == code_commune)].values.sum()))
        m75.append(int(dfAPA63['TOTAL_M75A'][(dfAPA63['ANNEE_REF'] == a) & (
            dfAPA63['CODE_COMMUNE'] == code_commune)].values.sum()))
        p75.append(int(dfAPA63['TOTAL_7584A'][(dfAPA63['ANNEE_REF'] == a) & (
            dfAPA63['CODE_COMMUNE'] == code_commune)].values.sum()))
        p84.append(int(dfAPA63['TOTAL_P85A'][(dfAPA63['ANNEE_REF'] == a) & (
            dfAPA63['CODE_COMMUNE'] == code_commune)].values.sum()))
        dom.append(int(dfAPA63['TOTAL_DOM'][(dfAPA63['ANNEE_REF'] == a) & (
            dfAPA63['CODE_COMMUNE'] == code_commune)].values.sum()))
        eta.append(int(dfAPA63['TOTAL_ETA'][(dfAPA63['ANNEE_REF'] == a) & (
            dfAPA63['CODE_COMMUNE'] == code_commune)].values.sum()))
    ...
    st.write(
        'Cette commune est issue de la fusion de Flat et Aulhat Saint Privat en 2016')
    for a in année[0:2]:
        total.append(int(dfAPA63['TOTAL'][(dfAPA63['ANNEE_REF'] == a) & (dfAPA63['CODE_COMMUNE'] == code_commune)].values.sum()) +
    ...
    st.write(
        'Cette commune est issue de la fusion de Vernet La Varenne et Chaméane en 2019')
    for a in année[0:5]:
        total.append(int(dfAPA63['TOTAL'][(dfAPA63['ANNEE_REF'] == a) &
                                          (dfAPA63['CODE_COMMUNE'] == code_commune)]
                                          .values.sum()) +
                     int(dfAPA63['TOTAL'][(dfAPA63['ANNEE_REF'] == a) &
                                          (dfAPA63['CODE_COMMUNE'] == 63078)]
                                          .values.sum()))
        fem.append(int(dfAPA63['TOTAL_FEM'][(dfAPA63['ANNEE_REF'] == a) & 
                                            (dfAPA63['CODE_COMMUNE'] == code_commune)]
                                            .values.sum()) +
                   int(dfAPA63['TOTAL_FEM'][(dfAPA63['ANNEE_REF'] == a) &
                                            (dfAPA63['CODE_COMMUNE'] == 63078)]
                                            .values.sum()))
        hom.append(int(dfAPA63['TOTAL_HOM'][(dfAPA63['ANNEE_REF'] == a) &
                                            (dfAPA63['CODE_COMMUNE'] == code_commune)]
                                            .values.sum()) +
                   int(dfAPA63['TOTAL_HOM'][(dfAPA63['ANNEE_REF'] == a) &
                                            (dfAPA63['CODE_COMMUNE'] == 63078)]
                                            .values.sum()))
        m75.append(int(dfAPA63['TOTAL_M75A'][(dfAPA63['ANNEE_REF'] == a) & (dfAPA63['CODE_COMMUNE'] == code_commune)].values.sum()) +
...
if choix == 'Par sexe':
    ...
    plt.bar(x=année, height=p84, bottom=np.array(
        m75) + np.array(p75), label='Plus de 84 ans')
    plt.legend(loc='lower left')
